...
| **M3** App shell — auth, workspaces, sessions, storage, library | 🟨 identity seam with dev + Clerk, per-user ownership on every row and route, isolation tested, sign-in UI behind a config flag. Clerk verified end to end against a real dev instance — sign-up, redirect, token-authenticated generation, correct ownership. **No workspaces; no production Clerk instance; `users.email` is blank until the session token is customised** |
